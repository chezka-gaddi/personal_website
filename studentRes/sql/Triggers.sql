...
ON Task
    ...
    BEGIN
        ...
        IF NEW.estDuration < 0 AND New.estDuration != '' THEN
...
ON Course
	FOR EACH ROW
    BEGIN
		IF NEW.instructor NOT LIKE '%[a-zA-z -]%' THEN
			SIGNAL SQLSTATE '45000'
...
ON User
    ...
    BEGIN
        ...
        IF NEW.GPA < 0 OR NEW.GPA > 4.0 AND NEW.GPA!='' THEN
			SIGNAL SQLSTATE '45000'
				SET MESSAGE_TEXT = 'GPA must be between 0.0 and 4.0';
		END IF;
	END;
    ...
    BEGIN
		IF NEW.instructor NOT LIKE '%[a-zA-z -]%' THEN
			SIGNAL SQLSTATE '45000'
				SET MESSAGE_TEXT = 'instructor cannot contain special characters or numbers.';
		END IF;
		IF NEW.creditHours < 0 OR NEW.creditHours > 5 AND NEW.creditHours!='' THEN
			SIGNAL SQLSTATE '45000'
				SET MESSAGE_TEXT = 'creditHours must be between 0 and 5';
		END IF;
END;   
