...
//would be better as it name it the same as what the user uploaded.
...
//if OK thus far, try to save
    if (!$message) {
        if (!is_uploaded_file($_FILES['fileToUpload']['tmp_name'])) {
            ...
            if (!move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) { // No error suppression so we can see the underlying error.
